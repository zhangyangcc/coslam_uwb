...
%% generate noise
% generate std_pose1 std_pose2
std_pose = [0.001 0.002 0.003 0.004 0.005 0.006 0.007];
std_distance = [0.001 0.002 0.003 0.004 0.005 0.006 0.007];

total_res = zeros(7,7);
[X,Y] = meshgrid(std_pose,std_distance);
for nn = 1:7
    for mm = 1:7
        tmp_total = 0;
for num = 1:3000
% generate delta_robot1_pose
delta_pose_robot1 = zeros(3,5);
for n = 1:5
    delta_pose_robot1(:,n) = pose_robot1(:,n+1) - pose_robot1(:,n);
end

% generate delta_robot1_pose_noise
% 标准差为0.1
for n = 1:5
    delta_pose_robot1_noise(:,n) = delta_pose_robot1(:,n) + std_pose(nn)*randn(3,1);
end

% generate robot1_pose_noise
pose_robot1_noise = zeros(3,6);
for n = 1:5
    pose_robot1_noise(:,n+1) = pose_robot1_noise(:,n) + delta_pose_robot1_noise(:,n);
end

% generate delta_robot2_pose
delta_pose_robot2 = zeros(3,5);
for n = 1:5
    delta_pose_robot2(:,n) = pose_robot2(:,n+1) - pose_robot2(:,n);
end

% generate delta_robot1_pose_noise
% 标准差为0.1
for n = 1:5
    delta_pose_robot2_noise(:,n) = delta_pose_robot2(:,n) + std_pose(nn)*randn(3,1);
end

% generate robot1_pose_noise
pose_robot2_noise = zeros(3,6);
for n = 1:5
    pose_robot2_noise(:,n+1) = pose_robot2_noise(:,n) + delta_pose_robot2_noise(:,n);
end

% generate distance_noise
for n = 1:6
    distance_noise(1,n) = distance(1,n) + std_distance(mm)*randn(1,1);
end
...
A = zeros(5,8);
for n =  2:6
    A(n-1,1) = -pose_robot1_noise(1,n);
    A(n-1,2) = -pose_robot1_noise(2,n);
    A(n-1,3) = pose_robot2_noise(3,n) - pose_robot1_noise(3,n);
    A(n-1,4) = -pose_robot1_noise(1,n)*pose_robot2_noise(1,n) - pose_robot1_noise(2,n)*pose_robot2_noise(2,n);
    A(n-1,5) = -pose_robot1_noise(2,n)*pose_robot2_noise(1,n) + pose_robot1_noise(1,n)*pose_robot2_noise(2,n);
    A(n-1,6) = pose_robot2_noise(1,n);
    A(n-1,7) = pose_robot2_noise(2,n);
    A(n-1,8) = -0.5*(distance_noise(1,n)^2-distance_noise(1,1)^2-transpose(pose_robot2_noise(:,n))*pose_robot2_noise(:,n)-transpose(pose_robot1_noise(:,n))*pose_robot1_noise(:,n)) - pose_robot1_noise(3,n)*pose_robot2_noise(3,n);
end
...
F(2) = distance_noise(1)^2 - (t(1)^2+t(2)^2+t(3)^2)/t(8)^2;
F(3) = -(t(1)*t(4)+t(2)*t(5))/t(8)^2+t(6)/t(8);
F(4) = -(t(2)*t(4)-t(1)*t(5))/t(8)^2+t(7)/t(8);
F(5) = distance_noise(1)^2 - (t(3)^2+t(6)^2+t(7)^2)/t(8)^2;
...
phi = asin(x(5));
res_trans = x(1:3)
res = norm(res_trans-translation);
tmp_total = tmp_total+res;
end
tmp_total = tmp_total/3000;
total_res(mm,nn) = tmp_total;
    end
end
surf(X,Y,total_res);